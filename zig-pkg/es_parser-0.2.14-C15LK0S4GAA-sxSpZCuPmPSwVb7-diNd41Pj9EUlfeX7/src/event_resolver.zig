...
fn resolveFullImpl(
    ...
    var sym_to_canonical = std.ArrayListUnmanaged(SymbolId).empty;
    ...
    var unresolved_refs = std.ArrayListUnmanaged(UnresolvedRef).empty;
    // (freed by scope_arena.deinit)
    ...
    while (ev_i < ev_visible) {
        ...
        .scope_open => {
            ...
            if (sp == stack.len) {
                ...
                for (undo_stacks[old_cap..]) |*u| u.* = .empty;
            }
            stack[sp] = sid;
            kind_stack[sp] = kind;
            node_stack[sp] = node;
